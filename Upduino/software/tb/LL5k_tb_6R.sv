...
logic clk;
     ...
     begin
          ...
          // Attempt set RXD=1 while TXD=1 => illegal
          do_write8_check(CONTROL_ADDR, 8'h40, 8'h02, 8'h80);
          do_read8_check (CONTROL_ADDR, 8'h00, 8'h80);

          // STATUS: ERR should be set after illegal operation, and clear-on-read (PZT_GO is 0 here)
          do_read8_check (STATUS_ADDR,  8'h00, 8'h40);
          do_read8_check (STATUS_ADDR,  8'h00, 8'h00);
// STATUS: PZT_GO mirrors CONTROL bit1
          ...
          // Manual debug write must be illegal while PZT_GO=1 (Policy 1)
          // Try to set RXD=1; expect rejection, CONTROL unchanged (=0x02), ERR sets and clears on read.
          do_write8_check(CONTROL_ADDR, 8'h40, 8'h02, 8'h02);
          do_read8_check (CONTROL_ADDR, 8'h00, 8'h02);

          // STATUS includes ERR (D6) and PZT_GO mirror (D1) => 0x40 | 0x02 = 0x42
          do_read8_check (STATUS_ADDR,  8'h00, 8'h42);
          // ERR clears on read, PZT_GO remains => 0x02
          do_read8_check (STATUS_ADDR,  8'h00, 8'h02);

          // Clear PZT_GO back to idle
          do_write8_check(CONTROL_ADDR, 8'h00, 8'h00, 8'h00);
          do_read8_check (CONTROL_ADDR, 8'h00, 8'h00);
// Clear all
          do_write8_check(CONTROL_ADDR, 8'h00, 8'h00, 8'h00);
