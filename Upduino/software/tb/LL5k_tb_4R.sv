...
logic clk;
     ...
     begin
          ...
          // Attempt set RXD=1 while TXD=1 => illegal; must not change CONTROL, and must set ERR flag
          do_write8_check(CONTROL_ADDR, 8'h40, 8'h02, 8'h80);
          do_read8_check (CONTROL_ADDR, 8'h00, 8'h80);

          // STATUS: ERR should now be set, and clear-on-read
          do_read8_check (STATUS_ADDR,  8'h00, 8'h40);
          do_read8_check (STATUS_ADDR,  8'h00, 8'h00);

          // POLICY 1 (Debug-lockout):
          //   - PZT_GO may be set only when TXD=0, RXD=0, PZT=0
          //   - TXD/RXD/PZT may be set only when PZT_GO=0
          //
          // Attempt to set PZT_GO while TXD is still active -> illegal, ERR sets
          do_write8_check(CONTROL_ADDR, 8'h02, 8'h02, 8'h80);
          do_read8_check (STATUS_ADDR,  8'h00, 8'h40);
          do_read8_check (STATUS_ADDR,  8'h00, 8'h00);

          // Clear TXD, then set PZT_GO -> legal
