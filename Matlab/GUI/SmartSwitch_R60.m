...
function BuildStatusBarLeftOnly(root, fig)
     ...
     left.RowHeight = {'1x'};
     left.ColumnWidth = {70, 120, 36, 55, 90, 100};
...
function OnTranslatePhysicsToRegisters(fig)
     % Convert button: Read Receive Switch ON/OFF Values, convert to hex, populate registers 0x23 (RXD START TIME) and 0x24 (RXD STOP TIME) in memory map
...
function BuildSigGenCard(parent)
     ...
     btnConnect.Tooltip = 'Download and configure Signal Generator parameters to the TG5011A (does not enable output).';
...
ddF0.ValueChangedFcn     = @(s,e)refreshDerived();
     ...
     function refreshDerived()
          ...
          % Calculate Burst Frame Duration = (transducer's period x Number of bursts) with scientific suffix
          transducerPeriod_s = 1 / f0_hz;  % Period = 1/frequency
          burstFrameDuration_s = transducerPeriod_s * round(edBurstN.Value);
...
function clkdiv = timebaseToClkDiv(timebase_us)
     % Convert timebase (us) to CLKDIV value
     % Mapping: 0.5 => 6, 1.0 => 12, 2.0 => 24, 5.0 => 60, 10.0 => 120
     ...
     elseif abs(timebase_us - 10.0) < tol
          ...
          % Default to 12 (1.0 us) if value doesn't match
          clkdiv = 12;
     end
end
...
function SS_TG5011A_Update(fig)
     ...
     try
          ...
          % Commands (mirrors the working Python baseline)
          cmds = { ...
          ...
          % Add burst cycle (N) command if valid value is available
          if isfinite(burst_n) && burst_n > 0
               cmds{end+1} = sprintf('BSTCOUNT %d', round(burst_n));
          end
          
          % Add LOCAL command at the end
          cmds{end+1} = 'LOCAL';
